...
function Singup() {
  return (
    ...
    <div id="" className="w-96 h-80">
      <div className="modal-box dark:bg-slate-800 dark:text-white">
          <form method="dialog">
          {/* if there is a button in form, it will close the modal */}
          <Link to='/' > <button className="btn btn-sm btn-circle btn-ghost dark:text-white  absolute right-2 top-2">✕</button></Link>
          </form>
          <h3 className=" pb-1font-bold text-lg">Login</h3>
          <br />
          <div>
          <label className="input input-bordered flex items-center gap-2">
                  <input type="text" className="dark:text-black grow" placeholder="Enter Full Name" />
              </label>
          <br />
              <label className="input input-bordered flex items-center gap-2">
          ...
          <div className='flex flex-row justify-between mt-5'>
              <div>
                  <button className='btn btn-sm text-white bg-pink-500'>SingUp</button>
              </div>
              <div className='py-2'>
                  <Login />
                   <a onClick={()=>document.getElementById("my_modal_3").showModal()} className=" px-4 py-1 text-blue-600 font-bold cursor-pointer">Login</a>
              </div>  
          </div>
      </div>
   </div>
